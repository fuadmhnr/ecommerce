            ...
            <div class="flex flex-wrap mb-24 -mx-3">
                <div class="w-full pr-2 lg:w-1/4 lg:block">
                            ...
                            @foreach ($categories as $category)
                                <li class="mb-4" wire:key="{{ $category->id }}">
                                    <label for="{{ $category->slug }}" class="flex items-center dark:text-gray-400 ">
                                        <input wire:model.live="selectedCategory" type="checkbox" class="w-4 h-4 mr-2" id="{{ $category->slug }}"
                                            value="{{ $category->id }}">
                            ...
                            @foreach ($brands as $brand)
                                <li class="mb-4" wire-key="{{ $brand->id }}">
                                    <label for={{ $brand->slug }} class="flex items-center dark:text-gray-300">
                                        <input type="checkbox" wire:model.live="selectedBrand" class="w-4 h-4 mr-2" id="{{ $brand->slug }}"
                                            value="{{ $brand->id }}">
                        ...
                        <ul>
                            ...
                            <li class="mb-4">
                                <label for="onSale" class="flex items-center dark:text-gray-300">
                                    <input type="checkbox" id="onSale" wire:model.live="onSale" value="1" class="w-4 h-4 mr-2">
                    ...
                    <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                        ...
                        <div class="w-16 pb-2 mb-6 border-b border-rose-600 dark:border-gray-400"></div>
                        <div>
                            <div class="font-semibold text-sm">{{ Number::currency($priceRange, 'IDR') }}</div>
                            <input type="range"
                                wire:model.live="priceRange"
                                class="w-full h-1 mb-4 bg-blue-100 rounded appearance-none cursor-pointer"
                                max="25000000" value="100000" step="100000">
                            <div class="flex justify-between ">
                                <span class="inline-block text-xs font-semibold text-blue-400 ">{{ Number::currency('100000', 'IDR') }}</span>
                                <span class="inline-block text-xs font-semibold text-blue-400 ">{{ Number::currency('25000000', 'IDR') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex flex-wrap items-center ">
                        @foreach ($products as $product)
                            <div class="w-full px-3 mb-6 sm:w-1/2 md:w-1/3" wire:key={{ $product->id }}>
                                <div class="flex flex-col h-[440px] border border-gray-300 dark:border-gray-700">
                                    ...
                                    <div class="flex flex-col flex-1">
                                        ...
                                        <div class="p-3 h-32">
                                            <div class="mb-2">
                                                ...
                                                <!-- Description container with line clamp -->
                                        ...
                                        <div wire:click.prevent="addToCart({{ $product->id }})" class="mt-auto p-4 border-t border-gray-300 dark:border-gray-700">
                                            <a href="#"
                                                class="flex items-center justify-center space-x-2 text-gray-500 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-300">
